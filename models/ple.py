...
class PLELayer(nn.Module):
    def __init__(self, input_dim, num_shared_experts, num_task_experts, num_tasks, expert_hidden_dims, dropout):
        ...
        self.task_gates = nn.ModuleList([Gate(input_dim, num_shared_experts + num_task_experts) for _ in range(num_tasks)])
    ...
    def forward(self, x):
        ...
        gated_outputs = []
        for i in range(self.num_tasks):
            selected_matrix = torch.cat((expert_outputs[i], expert_outputs[-1]), dim=1)
...
class PLE(nn.Module):
    def __init__(self, tasks, feature_dims, num_layers, num_shared_experts, num_task_experts, expert_hidden_dims, tower_hidden_dims, task_output_dims, dropout, embed_dim=0):
        ...
        for i in range(num_layers):
            expert_input_dim = input_dim if i==0 else expert_hidden_dims[-1]
            self.layers.append(PLELayer(expert_input_dim, num_shared_experts, num_task_experts, len(tasks), expert_hidden_dims, dropout))
    ...
    def forward(self, x):
        ...
        y = [x.clone() for _ in range(len(self.tasks)+1)] # list of inputs for each set of task experts and shared expert
